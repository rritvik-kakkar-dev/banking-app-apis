...
function TransferModal({ onClose, onSuccess }) {
    ...
    const handleTransfer = async () => {
        ...
        setLoading(true);
        try {
            await api.post("/api/user/transfer", {
                sourceAccountNumber: Number(accountNumber),
                destinationAccountNumber: Number(destinationAccount),
                amount: Number(amount)
            });
            toast.success(
                `₹${Number(amount).toLocaleString("en-IN")} transferred successfully to ${destinationAccount}`
            );
            ...
            setError("Failed to transfer amount. Try again.");
            setLoading(false);
        }
    };
            ...
            <div className="bg-[#fefefe] border border-[#e8e8e3] rounded-2xl p-6 w-96 shadow-xl">
                ...
                <div className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <h2 className="text-gray-900 font-bold text-lg">Transfer Amount</h2>
                    </div>
                    <button
                        onClick={onClose}
